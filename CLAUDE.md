...
## 画像の作り方（Claude Code環境）

**Claude Code 自体に画像生成機能は無いが、Codex CLI の標準画像生成ツールを呼び出して生成する。**
ChatGPTのサブスク枠で動き、APIキーは要らない。これが既定の手順。「生成したふり」をしない。

```bash
codex login status   # 「Logged in using ChatGPT」を確認してから着手する
```

手順の正本は `tamashiro-product-article` スキルの `references/claude-code-runtime.md`。
生成は同スキルの `scripts/generate_character_image.sh` を使う。要点だけ再掲する。

- **モデル名を書かない。** 「gpt-image-2.5で」と指定すると `OPENAI_API_KEY` を要求されて失敗する。
- **正本シートを参照画像として渡す。** Codexに `view_image` でローカルPNGを読ませ、画像生成ツールへ渡させる。
  参照なしで名前だけ似せた人物や一般人物を代替にしない。
- 生成後は **Readツールで画像を開き、正本シートと並べて目視照合する**。`CHARACTER_GUIDE.md` の「生成後の確認項目」を使う。
- WebPへ変換して `src/assets/blog/<slug>/` へ置く。リポジトリの `sharp` を使う。
- 比率は横長。**生成物を切って比率を合わせない。** 1536x1024（3:2）で出たものは3:2のまま使う。
- 画像を置いたら `heroImage` 追記 → プレースホルダー置換 → 証跡コメント追加 → 機械検査 → `npm run build` の順で仕上げる。

### 画像ハンドオフ（Codexが使えないときの退避）

`codex` が未ログイン・未インストール、**ChatGPTの利用枠切れ**、または記事だけ先に仕上げたい場合は、
記事本文を先に完成させ、画像は後日ユーザーが **ChatGPT（GPT Image 2）** で生成する。
枠切れは生成の途中で起きるため、**作れた分は設置し、残りだけを引き継ぐ**。
...
したがって、**画像ハンドオフへ退避して画像が未生成のままコミットする場合**は、次を厳守する。
...
### 引き継ぎファイルを作る（ハンドオフへ退避したときだけ）

記事ごとに `docs/image-handoff/<slug>.md` を作成し、以下を画像1枚ごとに記載する。
Codexで生成しきれた記事では不要（生成の記録は `logs/` に残す）。
