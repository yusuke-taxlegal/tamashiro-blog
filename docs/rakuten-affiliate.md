...
> [!success] 2026-09-22: 使える状態になりました
> 楽天アフィリエイトに `https://ysk.life` をサイト登録し、`stores.rakuten.enabled` を `true` にしました。
> あとは管理画面でリンクを作って記事や道具箱に貼るだけです。
...
## 1. 楽天側でやること（2026-09-22に完了）

1. 楽天会員IDで <https://affiliate.rakuten.co.jp/> にログインする。審査はなく、楽天IDがあれば使えます。
2. **サイト登録で `https://ysk.life` を登録する。** ← 完了
   楽天アフィリエイトのガイドラインでは、登録していないサイトへのリンク掲載は禁止です。ここを飛ばすと規約違反になります。
   登録内容: 順位1位／運営サイト名「玉城祐輔｜仕事と暮らしの、役立つ実践ノート。」／ジャンル「専門性日記」／商品ジャンル「家電」。
   変更は マイページ → サイト情報の登録 から。
3. 成果の受取方法（楽天キャッシュ／銀行振込）を確認しておく。 ← 未確認

> [!note] 「アフィリエイトID」は控えなくてよい
> Amazonの `tag=tamashirotool-22` のような固定IDは楽天には**ありません**。
> 管理画面が発行するリンクは `hb.afl.rakuten.co.jp/ichiba/<毎回変わる文字列>/` の形で、
> 商品ごとにIDが変わります（2026-09-22に2本発行して確認）。
> そのため、正しいリンクかどうかは**ホスト名**で判定しています。
...
## 2. サイト側でやること（2026-09-22に完了）

[`src/lib/affiliate.ts`](../src/lib/affiliate.ts) の `stores.rakuten.enabled` を `true` にしました。
今後もし楽天の掲載をやめるときは、ここを `false` に戻すだけです。
...
- [x] 楽天アフィリエイトの管理画面で ysk.life がサイト登録済み（2026-09-22）
