...
## スチームアイロン（Amazonアフィリエイト）の追記

2026-08-29、玉城本人から「スチームアイロンを使ってこのパンツのシワを伸ばしている」との情報があり、Amazonアフィリエイトリンク `https://amzn.to/4zEYgBf` を提供された。

追記した内容:

- 見出し「### シワは、スチームアイロンで伸ばしています」を、お手入れの節の末尾に追加
- 公式は「アイロン不要」と案内していることを先に書き、そのうえで玉城はスチームアイロンを使っている、という順序にした
- 熱の当て方（当て布、またはスチームを浮かせて当てる）は公式のお手入れ案内どおり
- リンクは `affiliate-product-button` ＋ `affiliate-product-disclosure` で設置し、紹介料が玉城個人に帰属することを明記した

あわせて修正した箇所:

- FAQ「アイロンは必要？」に「私も普段はかけていません」と書いていたが、本人の実際の運用と矛盾するため書き換えた（この一文は当初、本人未確認のまま書いてしまっていた）
- 「シワを伸ばすために手をかけたりしなくてよい」という記述も同様に矛盾するため、「クリーニングへ出すタイミングを考えなくてよい」へ変更した

**商品名は未記載。** この作業環境から `amzn.to` へアクセスできず（egress proxyでブロック）、リンク先の商品名・型番・商品画像URLを確認できなかった。
そのため既存記事の `affiliate-product-card`（商品画像・商品名が必要）は使わず、「次のスチームアイロンです」という表現とボタンだけにしている。
本人へ商品名を確認できたら、他の商品記事と同じ商品カード（Amazon提供元のリモート画像＋商品名）へ差し替える。
...
6. スチームアイロンの商品名・型番を本人へ確認し、商品カード（Amazon提供元のリモート画像＋商品名）へ差し替える。
7. 道具箱（`src/components/ToolboxPage.astro`）へWWSを追加するかは別作業として判断する。記事公開のコミットには混ぜない。
